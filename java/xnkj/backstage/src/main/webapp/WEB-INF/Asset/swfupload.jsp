...
<style>
	...
	.eidt_uphoto .no {
		background: url("<%=request.getContextPath()%>/assets/images/upload_pic.jpg") no-repeat scroll center center #FBFBFB;
		border: 1px solid #CCCCCC;
		height: 98px;
		overflow: hidden;
		text-indent: -2000em;
	...
	.eidt_uphoto .del{
		...
		width:15px;
		height:15px;
		background:url("<%=request.getContextPath()%>/assets/images/upload_del.png") no-repeat;
		right:1px;
		top:1px;
		overflow:hidden;
		text-indent:-2000em;
	...
	/*上传选择按钮*/
	#btupload,.addnew{
		background: url("<%=request.getContextPath()%>/js/swfupload/images/swfBnt.png") no-repeat; float:left; margin-right:10px;width:75px; height:28px; line-height:28px;font-weight:700; color:#fff;
	}
	#btupload{
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		Wind.use("swfupload","<%=request.getContextPath()%>/js/swfupload/handlers.js",function(){
			swfu = new SWFUpload({
				flash_url:"<%=request.getContextPath()%>/js/swfupload/swfupload.swf",
				upload_url:"/Public/uploadPicture",
		...
		<div class="pop_cont">
			<div class="">
				<div class="edit_uping">
					...
					<span class="num">最多上传<em> 1</em> 个附件,单文件最大 <em>80960 KB</em>，<em style="cursor: help;" title="可上传格式：jpg,jpeg,gif,png,bmp">支持格式？</em></span>
				</div>
