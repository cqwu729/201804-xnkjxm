...
<div class="wrap js-check-wrap">
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="#">编辑公告咨询</a></li>
	</ul>
			...
			<div class="span9">
				<table class="table table-bordered">
						...
						<td>
							<select name="columnId" id="columnId">
								<c:forEach items="${columns}" var="column">
									<option value="${column.id}" <c:if test="${column.id == notice.column.id}">selected</c:if>>${column.name}</option>
								</c:forEach>
							</select>
						</td>
					</tr>
					<tr>
						<th>新闻名称</th>
						<td>
							<input type="hidden" style="width:400px;" name="id" id="id" required value="${notice.id}" placeholder="请输入新闻名称"/>
							<input type="text" style="width:400px;" name="name" id="name" required value="${notice.name}" placeholder="请输入名称"/>
					...
					<tr>
						<th>内容</th>
						<td>
							<script type="text/plain" id="content" name="content" style="width:100%">${notice.content}</script>
                            </td>
                            </tr>
				</table>
			</div>
					...
					<tr>
						<td>
							<div style="text-align: center;">
								<input type="hidden" name="picture" id="thumb" value="${notice.picture}">
								<a href="javascript:void(0);" onclick="flashupload('thumb_images', '附件上传','thumb',thumb_images,'1,jpg|jpeg|gif|png|bmp,1,,,1','','','');return false;">
									<c:if test="${not empty notice.picture}">
										<img src="${notice.picture}" id="thumb_preview" width="135" style="cursor: hand" />
									</c:if>
									<c:if test="${empty notice.picture}">
