...
function att_show(serverData,file)
{
	...
	//附件aid
	var id = data[0].data.picture;
	//附件地址
	var src = data[0].data.picture;
	//附件类型
	var ext = data[0].data.ext;
	//附件名称
	var filename = data[0].data.name;
	//上传失败，提示上传信息
	if(id == 0) {
		try{
	...
	if(ext == 1) {
		...
		img = '<div class="get selected" style="width:200px;" id="aid-'+id+'"><a class="del" href="javascript:;">删除</a>' +
		'<input type="text" class="J_file_desc" style="width:200px;" name="flashatt['+id+'][desc]" placeholder="请输入描述" value="'+filename+'"  style="width:68px"></div>';
	} else {
